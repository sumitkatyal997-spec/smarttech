        ...
        <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="name" value="{{ old('name', $product->name) }}" class="w-full rounded border px-3 py-2" required>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1">SKU (optional)</label>
                    <input name="sku" value="{{ old('sku', $product->sku) }}" class="w-full rounded border px-3 py-2">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="tracking" class="w-full rounded border px-3 py-2" required>
                        <option value="qty" @selected(old('tracking', $product->tracking) === 'qty')>Quantity</option>
                        <option value="serial" @selected(old('tracking', $product->tracking) === 'serial')>Serialized (per unit)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="reorder_level" type="number" min="0" value="{{ old('reorder_level', $product->reorder_level) }}" class="w-full rounded border px-3 py-2">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Notes</label>
                <textarea name="notes" rows="3" class="w-full rounded border px-3 py-2">{{ old('notes', $product->notes) }}</textarea>
            </div>
