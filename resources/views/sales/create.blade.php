...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-semibold">New Sale (Stock Out)</h1>
        ...
        <form method="POST" action="{{ route('sales.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input name="sold_at" type="date" value="{{ old('sold_at', now()->format('Y-m-d')) }}" class="w-full rounded border px-3 py-2" required>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1">Customer</label>
                    <select name="customer_id" class="w-full rounded border px-3 py-2">
                        <option value="">—</option>
                ...
                <div>
                    ...
                    <input name="invoice_no" value="{{ old('invoice_no') }}" class="w-full rounded border px-3 py-2" placeholder="Optional">
                </div>
            </div>
            ...
            <div class="border rounded">
                    ...
                    <table class="min-w-full text-sm" id="items-table">
                        <thead class="text-left text-gray-600">
                            <tr>
                                ...
                                <th class="py-2 pr-4 w-40">Unit price</th>
                        ...
                        <tbody>
                            @for ($i = 0; $i < 3; $i++)
                                <tr class="border-t align-top">
                                    ...
                                    <td class="py-2 pr-4">
                                        <input name="items[{{ $i }}][unit_price]" type="number" step="0.01" min="0" class="w-32 rounded border px-3 py-2" value="0">
                                    </td>
                                    <td class="py-2 pr-4">
                ...
                <div class="px-4 py-3 border-t text-sm text-gray-600">
                    For serialized products, **Qty is ignored** and we use the serials list (each serial becomes 1 sold unit).
                </div>
            </div>

            <button class="px-4 py-2 rounded bg-gray-900 text-white hover:bg-gray-800" type="submit">Save Sale</button>
        </form>
    </div>
